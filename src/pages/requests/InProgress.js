...
export default () => {
    ...
    const { requests, currentPage, loading, filter } = useSelector(state => state.request);

    useEffect(() => {
        dispatch(updateFilter({ status: "progress" }))
    ...
    return (
        <>
            ...
            <RequestsTable requests={requests} handleUpdate={handleUpdate} previewImage={previewImage}  status="resolved" />
            <ConfirmationModal
                show={show}
